...
def run_inference_experiment(refinement_depth, baseline_ckpts_path, init_thetas=[1.0, -1.5]):
    """
    Uses the baseline ckpts (ground truth data) to run kernel_inference_parallel.py iteratively
    for increasing refinement depths.
    """
    exp_name = f"nonconvex_underparam_depth{refinement_depth}"
    output_root = f"output/{exp_name}"
    os.makedirs(output_root, exist_ok=True)

    # 1) Iteratively run kernel_inference_parallel.py for each depth
    output_dir = os.path.join(output_root, "inferred_kernels")
    os.makedirs(output_dir, exist_ok=True)

    for depth in range(1, refinement_depth + 1):
        kernel_inference_cmd = (
            f"python kernel_inference_parallel.py "
        ...
        print(f"Running kernel inference for depth {depth}")
...
if __name__ == "__main__":
    ...
    # set manual seed for reproducible data
    np.random.seed(128)
    random.seed(128)
    ...
    baseline_model, baseline_ckpts_path = generate_baseline_data(
        ...
        init_thetas=[-1.9, 1.9]  # consistent init for baseline
    )
